...
export class APIFarmDataService {
    ...
    public async genProductsForFarm(farmId: string): Promise<IFarmProduct[]> {
        ...
        return farmProductsColl.find({ farm_id: new ObjectId(farmId) }).toArray();
    }
    ...
    public async genPromotionalFarmProducts(
        ...
        const farmProductsColl = await this.genFarmProductsCollection();
        const tagFilters = API_CONFIG.promotionalProductTags.map(tag => ({ tags: tag }));
    ...
    private async genFarmProductsCollection(): Promise<Collection<IFarmProduct>> {
